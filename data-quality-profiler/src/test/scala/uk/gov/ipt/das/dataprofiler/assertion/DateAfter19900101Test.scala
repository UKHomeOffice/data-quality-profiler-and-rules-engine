...
class DateAfter19900101Test extends AnyFunSpec with SparkSessionTestWrapper {
  it("DateAfter19900101Test assertion calculated correctly") {
    ...
    val testDataset = Seq(
      recordFromValue("RECORD1", "1989-07-13"),
      recordFromValue("RECORD2", "XXXXXX"),
      recordFromValue("RECORD3", null),
      recordFromValue("RECORD4", "2021-07-13") ,
      recordFromValue("RECORD5", "1800-07-13")
    )
    ...
    val results =
      ProfilerConfiguration(
        ...
        rules =
          FieldBasedMask(BuiltInFunction(name = "DateAfter19900101"))
    ...
    assert(spark.sql(
      s"""SELECT featureValue FROM $assertionsView WHERE recordId = 'RECORD1' AND
         | featureName = 'DQ_DateAfter19900101'""".stripMargin).first().getString(0) === "false")
    ...
    assert(spark.sql(
      s"""SELECT featureValue FROM $assertionsView WHERE recordId = 'RECORD4' AND
         | featureName = 'DQ_DateAfter19900101'""".stripMargin).first().getString(0) === "true")
