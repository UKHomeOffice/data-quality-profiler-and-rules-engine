...
class NotBlankTest extends AnyFunSpec with SparkSessionTestWrapper {

  it("NotBlank assertion calculated correctly and handles empty and nulls") {
    ...
    val results =
      ProfilerConfiguration(
        ...
        rules =
          FieldBasedMask(BuiltInFunction(name = "NotBlank"))
    ...
    assert(spark.sql(
      s"""SELECT featureValue FROM $assertionsView WHERE recordId = 'SOME_ID_1' AND
         | featureName = 'DQ_NotBlank'""".stripMargin).first().getString(0) === "true")

    assert(spark.sql(
      s"""SELECT featureValue FROM $assertionsView WHERE recordId = 'SOME_ID_2' AND
         | featureName = 'DQ_NotBlank'""".stripMargin).first().getString(0) === "false")
