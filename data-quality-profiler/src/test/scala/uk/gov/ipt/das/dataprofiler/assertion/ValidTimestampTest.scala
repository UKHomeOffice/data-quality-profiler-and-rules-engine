...
class ValidTimestampTest extends AnyFunSpec with SparkSessionTestWrapper {
  it("ValidTimestamp assertion calculated correctly") {
    ...
    val results =
      ProfilerConfiguration(
        ...
        rules =
          FieldBasedMask(BuiltInFunction(name = "ValidTimestamp"))
    ...
    assert(spark.sql(
      s"""SELECT featureValue FROM $assertionsView WHERE recordId = 'RECORD1' AND
         | featureName = 'DQ_ValidTimestamp'""".stripMargin).first().getString(0) === "false")
    ...
    assert(spark.sql(
      s"""SELECT featureValue FROM $assertionsView WHERE recordId = 'RECORD4' AND
         | featureName = 'DQ_ValidTimestamp'""".stripMargin).first().getString(0) === "true")
