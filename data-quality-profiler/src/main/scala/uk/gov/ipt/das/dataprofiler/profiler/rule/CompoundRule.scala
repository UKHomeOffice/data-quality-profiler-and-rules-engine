...
case class CompoundRule(definitionName: String,
    ...
    profilableRecordSets.filter(filterByRecordSets).recordSets.toSeq.map {
      case (recordSet: String, records: FlattenedRecords) =>
        recordSet -> records.filterByPaths(filterByPaths).records.flatMap { record =>
          val profilers = rules.flatMap { case (path, rule) =>
            val filter = Option(Seq(Pattern.compile(path.replace("[]", "\\[\\]"))))
            val filteredRecord = record.filterByPaths(filter)
            filteredRecord.flatValues.map { flatValue =>
                  feature.FeaturePoint(
                    recordId = filteredRecord.id,
                    path = flatValue.flatPath,
                    originalValue = flatValue.recordValue.valueAsString,
                    feature = BuiltInFunction(rule).getFeatureOutput(flatValue.recordValue),
                    recordSet = recordSet,
                    additionalIdentifiers = filteredRecord.additionalIdentifiers
                  )
            } :+ // append path exists in record check - currently will fail if path is not present.
              FeaturePoint(
                recordId = record.id,
                path = path,
                originalValue = "",
          ...
          // currently not returning sub-assertions
//          profilers ++
            Seq(
              FeaturePoint(
                recordId = record.id,
